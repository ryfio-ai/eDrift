...
export default function OnboardChargerPage() {
  ...
  const obcProducts = products.filter(p => p.category === "On Board Charger");
  const [currentIdx, setCurrentIdx] = useState(0);

  React.useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIdx(prev => (prev + 1) % obcProducts.length);
    }, 4000);
    ...
    <div className="pt-32 pb-24 px-6 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-32">
          ...
          {/* Tab content — CSS opacity transition instead of AnimatePresence */}
          <div
            key={activeTab}
            style={{ animation: "heroFadeIn 0.25s ease both" }}
          >
